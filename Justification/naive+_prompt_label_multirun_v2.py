...
DEFAULT_PROMPT_TEMPLATE = (
    "Act as an expert of forex trading holding USD and CHN. "
    "Based on the article {article}, will you buy USD, sell USD or hold in the short term? "
    "Answer in one token: buy, sell, or hold."
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Label news with DeepSeek using a naive prompt, with multi-run output, per-run/per-label seeds, "
            "and one combined Excel workbook."
        )
    )
    ...
    parser.add_argument("--text_column", type=str, default="News", help="Column sent to the model")
    ...
    parser.add_argument(
        ...
        type=str,
        default="per_label",
        choices=["per_label", "per_run"],
        help=(
            "'per_label' gives each label generation its own seed derived from the run base seed. "
            "'per_run' reuses one seed for the full run."
        ),
    )
    parser.add_argument(
        "--base_seed",
        type=int,
        default=None,
        help="Optional initial base seed. If omitted, Unix time in milliseconds is used.",
    )
    parser.add_argument(
        "--prompt_template",
        type=str,
        default=DEFAULT_PROMPT_TEMPLATE,
        help="Prompt template containing '{article}'.",
    )
    parser.add_argument(
        "--input_encoding",
        type=str,
    ...
    parser.add_argument(
        "--save_raw_text",
        action="store_true",
        help="Also store raw model text for each run in the Excel output. Off by default to reduce memory usage.",
    )
    parser.add_argument(
        "--max_retries",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Pause after each successful request",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
...
def map_label_to_numeric(label_str: str) -> int:
    ...
    return mapping.get(label_str.strip().lower(), 0)


def extract_text_label(raw_text: Any) -> str:
...
def label_news_item(text_item: str, prompt_template: str, temperature: float, seed: int) -> str:
    prompt = prompt_template.format(article=text_item)
...
def get_or_create_run_metadata(
    ...
    for run_number in range(1, num_runs + 1):
        if run_number in by_run and pd.notna(by_run[run_number].get("base_seed")):
            ...
            base_seed = requested_seed if requested_seed is not None and run_number == 1 else get_unix_seed(run_number - 1)
            meta = {
...
def build_summary_df(output_df: pd.DataFrame, num_runs: int) -> pd.DataFrame:
    ...
    if not label_matrix.empty:
        ...
        summary["count_sell_neg1"] = (label_matrix == -1).sum(axis=1)
        summary["count_hold_0"] = (label_matrix == 0).sum(axis=1)
        summary["count_buy_1"] = (label_matrix == 1).sum(axis=1)
        ...
        summary["count_sell_neg1"] = 0
        summary["count_hold_0"] = 0
        summary["count_buy_1"] = 0
...
def main() -> None:
    ...
    prompt_text = args.prompt_template
        ...
        for row_idx in range(start_row, total_rows):
            if args.stop_after_api_calls is not None and successful_api_calls >= args.stop_after_api_calls:
                state_payload = build_state_payload(
                    ...
                    f"Stopped after {successful_api_calls} successful API calls",
                )
                run_metadata[run_idx]["status"] = "paused_manual_budget_cap"
                save_progress(output_df, run_metadata, output_file, checkpoint_file, log_file, state_payload, args, prompt_text, text_col, write_excel=True)
            ...
            try:
                ...
                text_label = extract_text_label(raw_text)
                numeric_label = map_label_to_numeric(text_label)
                output_df.at[row_idx, label_col] = numeric_label
                if raw_col is not None:
            ...
            if processed_since_save >= args.save_every:
                ...
                save_progress(output_df, run_metadata, output_file, checkpoint_file, log_file, state_payload, args, prompt_text, text_col, write_excel=False)
                processed_since_save = 0
