...
PROMPT_TEMPLATE = """Act as an expert of forex trading holding USD and CHN. Based on the article {article}, will you buy USD, sell USD or hold in the short term? Answer in one token: buy, sell, or hold."""
...
def label_news_item(news_item):
    """
    Sends a news item to the DeepSeek API using the simplified prompt template 
    and returns the label (buy/sell/hold).
    """
...
def main():
    ...
    output_df = df.copy()
    output_df["Naive + Prompt Label"] = numeric_labels  # Only numeric values: 0, -1, 1
    ...
    print(f"Buy (1): {numeric_labels.count(1)}")
    print(f"Sell (-1): {numeric_labels.count(-1)}")
    print(f"Hold (0): {numeric_labels.count(0)}")
